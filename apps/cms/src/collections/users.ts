 ...
 *
 * Чего здесь нет и почему. Ограничение частоты на ключ (Ч-14, задача Э6-03)
 * живёт в `src/http/api-rate-limit.ts` — обёрткой над файлами маршрутов, а не
 * хуком коллекции: хук выполняется и на Local API, то есть считал бы обращения
 * рендера `apps/web`, а не запросы внешнего клиента. Нет также 2FA для
 * администраторов (ТЗ §11, этап 7) и запрета удалить последнего администратора
 * (отдельная защита от самоблокировки, требует запроса к базе в хуке).
 *
