...
export function planCollectionNode(input: {
  ...
  // Условие C3: год в адресе ежегодного праздника. Проверяется по СВОЕМУ
  // сегменту, а не по всему пути: сегменты родителей проверены при их
  // сохранении, и отказывать записи за адрес родителя означало бы отказ, который
  // редактор не может исправить в своей форме.
