...
describe('card-images: хранение и отдача файлов', () => {
  ...
  it('хуки пайплайна подключены на всех трёх фазах', () => {
    expect(CardImages.hooks?.beforeChange).toHaveLength(1);
    // Два хука в afterChange, и порядок значим: сначала запись подготовленных
    // файлов, потом уборка прежних. Запись перенесена сюда из beforeChange
    // (находка ревизии от 2026-08-22): до записи документа в публичном
    // пространстве не должно появляться ни одного файла.
    expect(CardImages.hooks?.afterChange).toHaveLength(2);
...
describe('cards: поле image', () => {
  ...
  it('хуки изображения добавлены к общим, а не заменяют их', () => {
    ...
    expect(Cards.hooks?.beforeValidate).toHaveLength(2);
