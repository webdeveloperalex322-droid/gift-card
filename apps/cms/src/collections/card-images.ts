...
export const CardImages: CollectionConfig = {
  ...
  admin: {
    ...
    description:
      ...
      'постоянны: правка названия их не меняет. Удалить изображение можно только после ' +
      'того, как на него не ссылается ни одна карточка: иначе у карточки остались бы ' +
  ...
  access: {
    ...
    // URL файла постоянен — удаление это решение, а не уборка. И даже человеку
    // удаление ОТКЛОНЯЕТСЯ, пока на изображение ссылается хоть одна карточка
    // (`beforeDelete` в `../images/upload-hooks.ts`): связь в базе каскадная,
    // поэтому иначе поле карточки обнулилось бы молча, а зеркало путей осталось
    // бы с ключами удалённых файлов.
