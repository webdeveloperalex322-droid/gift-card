...
export type ContentRuleCode =
  ...
  // Находка ревизии от 2026-08-29: массовое УДАЛЕНИЕ опубликованных записей.
  // Пакетный гейт стоял только на `update` по `where`, а `delete` по `where` не
  // был закрыт ничем. См. `assertBulkDeleteAllowed`.
  ...
  // Задача Э5-01, добор по ревизии 2026-08-29: открытие в index,follow у
  // записи, чьи title или description совпадают с чужими. Правило живёт в
  // `./meta-duplicates.ts` (`assertMetaUniqueForIndex`), код отказа — здесь,
  // как и у остальных условий индексации: внешний клиент разбирает отказы по
  // коду, а не по тексту. Это условие п. 5.1 «уникальные title/H1/вводный
  // текст», проверяемое в тот момент, когда решение применяется.
...
/**
 * Проверяет пакетное УДАЛЕНИЕ (Payload: `delete` по `where`).
 *
 * Правило простое: в выборке не должно быть ни одной ОПУБЛИКОВАННОЙ записи.
 *
 * Почему это отдельное правило, а не следствие уже написанных. Пакетный гейт
 * {@link assertBulkChangeAllowed} стоит на `update` по `where` и до ревизии
 * 2026-08-29 закрывал ровно его: `delete` по `where` не проверялся ничем, а
 * `beforeDelete` есть только у подборок и ловит совсем другое (вложенные узлы).
 * Значит, одна операция удаляла выборку опубликованных карточек — без решения
 * «301 или 404» по каждому адресу, того самого, которого требует
 * `bulk-withdrawal-forbidden` при обычном снятии с публикации. Удаление тут
 * хуже снятия: снятая запись хотя бы остаётся в базе вместе со своим путём, а
 * удалённая не оставляет ни адреса, ни следа, по которому редирект можно было бы
 * поставить задним числом.
 *
 * Черновики удаляются пакетом свободно: у записи, которая никогда не
 * публиковалась, публичного URL не было, и решать о его судьбе нечего. Тот же
 * критерий, что у `canDeleteContent` для `ai-editor`.
 *
 * ЧТО ЭТО ПРАВИЛО НЕ ЗАКРЫВАЕТ, СКАЗАНО ПРЯМО: поштучное удаление опубликованной
 * записи (`delete` по id) остаётся возможным для `admin` и решения о судьбе URL
 * не требует. Это осознанно оставленный пробел, а не недосмотр: поле `withdrawal`
 * живёт в самой записи и заполняется при СМЕНЕ СТАТУСА, а у удаления записи, в
 * которую его писать, уже нет. Приличный порядок — сначала снять с публикации с
 * решением, потом удалить — сегодня держится на инструкции, а не на коде.
 * Заведено строкой Э5-08-A в `docs/otkrytye-voprosy.md`.
 *
 * @param publishedInSelection сколько опубликованных записей попало в выборку;
 *   считает вызывающий, потому что для этого нужна база
 * @throws ContentRuleError
 */
export function assertBulkDeleteAllowed(input: {
  readonly publishedInSelection: number;
}): void {
  if (input.publishedInSelection <= 0) {
    return;
  }
  fail(
    'bulk-delete-published-forbidden',
    `В выборку на удаление попало опубликованных записей: ${String(input.publishedInSelection)}. ` +
      'Массовое удаление опубликованных страниц запрещено: адрес каждой уже известен ' +
      'поисковику, и по каждому нужно СВОЁ решение — 301 на замену, 410 или 404 (ТЗ §8.2). ' +
      'Одной операцией такое решение принять нельзя: общий путь замены — это редирект ' +
      'нескольких удалённых страниц на один адрес, запрещённый п. 23 SEO ТЗ, а общий 404 — ' +
      'решение, принятое не глядя. Порядок: снимите записи с публикации по одной, выбирая ' +
      'судьбу URL для каждой, и только потом удаляйте.',
  );
}
