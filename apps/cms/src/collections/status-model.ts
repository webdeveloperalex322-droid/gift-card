 ...
 *
 * `image` перечислен, хотя поля загрузки в коллекции `cards` пока нет: оно
 * появится вместе с коллекцией файлов (Э2-04). Требование к отсутствующему полю
 * пропускается {@link missingReviewFields} — иначе ни одна карточка не смогла бы
 * дойти до `review` до конца этапа 2. Как только поле появится в схеме,
 * требование включится само; это закреплено тестом, поэтому «включить потом»
 * не забудется.
 */
    ...
    if (input.missingForReview.length > 0) {
      ...
      fail(
        ...
        `Запись не готова: не заполнено — ${list}. Валидация полноты стоит на входе в ` +
          'review (ТЗ §8.2): страница без изображения, alt, подписи или привязки к ' +
          'подборкам не должна доходить до проверки человеком, а тем более до индекса.',
      );
    }
  }
 ...
 *
 * Ограничение содержательное, а не технологическое: пакет по решению Ч-07 — это
 * «выборка, которую человек выбрал сам». Список в тысячи id — это уже не выбор
 * человека, а тот же фильтр из кода, только переписанный перечислением. Значение
 * согласовано с админкой Payload: на странице списка выбирается не больше сотни
 * строк, поэтому предел не мешает штатной работе.
 */
...
export interface BulkIntent {
  ...
  readonly kind: 'index' | 'other' | 'publish';
}
 ...
 *
 * Смена статуса `draft ↔ review` пакетом разрешена без ограничений на выборку
 * (ТЗ §8.5): она не открывает ни одной страницы для поиска.
 *
 * @throws ContentRuleError
 */
export function assertBulkChangeAllowed(input: {
  ...
  if (!isAdmin(user)) {
    fail(
      ...
      'Пакетная публикация и пакетное включение index,follow доступны только роли admin ' +
        'из админки (решение Ч-07). Массовая публикация по расписанию, по вызову API ' +
        'сервисным аккаунтом или из кода остаётся запрещённой.',
    );
  }
  ...
  if (ids === null) {
    fail(
      'bulk-requires-explicit-selection',
      'Пакетная операция применяется только к ЯВНО выбранным записям (решение Ч-07: ' +
        '«выборка, которую выбрал сам»). Условие вида «все записи в review» — это ' +
        'массовая публикация по фильтру, и она запрещена: человек не видел, что именно ' +
        'уйдёт в индекс.',
    );
  }
  ...
  return { ids, kind: publishing ? 'publish' : 'index' };
}
