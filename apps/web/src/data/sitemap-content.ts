 ...
 *
 * Карта сайта собирается НА ЗАПРОСЕ: обход читает все опубликованные записи
 * (пачками по `SITEMAP_SCAN_BATCH`) и спрашивает предикат «непуст» у каждого
 * узла таксономии. Обход ОДИН на все четыре файла и на весь срок
 * `SITEMAP_CACHE_SECONDS` — см. {@link buildSitemapModel} и
 * `../seo/sitemap-memo.ts`. Это осознанный выбор задачи Э4-04 против генерации
 * файлов с последующей перегенерацией:
 *
 *   - у сгенерированного файла окно устаревания НЕОГРАНИЧЕННОЕ: он живёт до
 *     следующей перегенерации, и всё это время карта сайта может содержать
 *     адреса, которые уже отвечают 404 или уже закрыты `noindex`. Требование
 ...
 *
 * Память процесса этому выбору не противоречит: её срок равен `max-age`, уже
 * объявленному в `Cache-Control` тех же файлов, то есть окно устаревания
 * остаётся ровно тем, которое сайт и так разрешил кешу клиента и посредника.
 *
 * Граница пересмотра названа явно: когда ОДИН обход перестанет укладываться в
 * приемлемое время ответа (ориентир — десятки тысяч карточек), выбор меняется на
 * генерацию файлов, и вместе с ним появляется контракт перегенерации.
 */
...
import { createSitemapMemo } from '../seo/sitemap-memo.js';
import {
...
/**
 * Общая память процесса под собранную модель.
 *
 * Существует ровно затем, чтобы один проход краулера по четырём файлам карты
 * сайта обходил каталог ОДИН раз. Срок — {@link SITEMAP_CACHE_SECONDS}, то есть
 * тот же, что объявлен заголовком `Cache-Control` этих файлов; второй константы
 * срока нет (обоснование — `../seo/sitemap-memo.ts`).
 */
const memo = createSitemapMemo<SitemapModel>(SITEMAP_CACHE_SECONDS);

/**
 * Что различает модели между собой — хост, из которого собираются все
 * абсолютные адреса карты.
 *
 * На сайте он один (ровно один хост, «Правила URL»), поэтому ключ практически
 * постоянен. Он всё же вычисляется, а не опускается: смоук и тест зовут сборку
 * с подставленным окружением, и молча получить чужую модель они не должны.
 */
function memoKey(env: SharedEnv | undefined): string {
  return env === undefined ? '(окружение процесса)' : (env['SITE_URL'] ?? '');
}

/**
 * Полная карта сайта из текущего состояния базы.
 *
 * ОБХОД ОБЩИЙ. Функцию зовут четыре независимых маршрута
 * (`/sitemap.xml`, `-sections`, `-cards-N`, `-images-N`), и до правки по
 * вердикту `reviewer` от 2026-08-28 каждый читал весь каталог сам: адреса
 * публичные, названы в `robots.txt`, ограничения частоты у них нет — повторный
 * запрос стоил ещё одного полного обхода. Теперь модель живёт в памяти процесса
 * ровно тот срок, который объявлен в `Cache-Control` этих же файлов.
 *
 * Свежесть при этом не ухудшилась: окно устаревания того же размера уже было
 * объявлено заголовком, и ответ разрешено держать в кеше клиента и посредника
 * столько же.
 *
 * @throws Error если `SITE_URL` не задан либо если обход не сходится (см.
 *   `./content.ts`). Порча данных ОДНОЙ записи исключением не является: и
 *   негодный путь, и негодная дата исключают её с причиной (`../seo/sitemap.ts`).
 *   Отказ НЕ запоминается: следующий запрос пробует снова.
 */
export async function buildSitemapModel(env?: SharedEnv): Promise<SitemapModel> {
  return memo.get(memoKey(env), async () => sitemapModelFrom(await collectSitemapFacts(env), env));
}
