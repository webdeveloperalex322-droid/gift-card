...
/**
 * Родительский узел по идентификатору.
 *
 * Запрос списком, а не `findByID`, ровно по одной причине: неопубликованный
 * родитель обязан прийти ПУСТЫМ результатом, а не исключением о запрете доступа.
 * Различать «нет записи» и «нельзя читать» публичному рендеру нечем и незачем:
 * ссылки на неё всё равно не будет.
 */
export function parentCollectionQuery(
  parentId: RecordId | null,
): PublicFindQuery<'collections'> | null {
  if (parentId === null) {
    return null;
