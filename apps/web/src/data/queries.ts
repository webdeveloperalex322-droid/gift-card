 ...
 * Э3-05/Э3-06, MAJOR). Окно соседей это исправляет по построению: блок карточки
 * — непрерывный отрезок общего порядка вокруг неё, поэтому «быть соседом» —
 * отношение взаимное, и каждая карточка темы попадает в блок своего соседа.
 * Свойство проверяется тестом на наборе, заметно большем предела блока
 * (`./data-access.test.ts`).
 *
 ...
 * добирает другая. Из этого правила и следует достижимость (решение Ч-04-8): при
 * пределе не меньше двух каждая непустая половина получает хотя бы одну позицию,
 * поэтому непосредственный сосед карточки попадает в её блок ВСЕГДА, а значит и
 * она — в его.
 *
