 ...
 *   - **директивы робота.** С задачи Э4-01 она вычисляется в одном месте —
 *     `../seo/robots-directive.ts`, куда переехала и прежняя `robotsForPage`, и
 *     сам тип `RobotsDirective`. Здесь остался только НОМЕР страницы, который
 *     туда передаётся фактом: пока правило жило рядом с адресами, каждый шаблон
 *     складывал итоговую директиву сам, и sitemap (Э4-04) не имел одного ответа
 *     на вопрос «закрыта ли эта страница»;
 *   - **исключения страниц пагинации из sitemap.** Это состав sitemap, задача
 *     Э4-04; исключение обязано выводиться данными — директивой, посчитанной
 *     единственным разрешателем;
