 ...
 *      фильтры — всегда `noindex`). Сама директива считается не здесь: с задачи
 *      Э4-01 её вычисляет единственный разрешатель `../seo/robots-directive.ts`,
 *      а этот модуль отвечает на вопрос «активен ли фильтр»
 *      ({@link hasActiveFilter}) — то есть отдаёт ФАКТ, а не готовое значение.
 *      Прежняя `robotsForFilteredView` переехала туда целиком: пока правило
 *      «фильтр закрывает» жило здесь, а правило «страница 2+ закрывает» — в
 *      `./pagination.ts`, итоговую директиву складывал каждый шаблон сам;
