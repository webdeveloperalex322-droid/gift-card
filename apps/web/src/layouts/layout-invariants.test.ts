...
/**
 * Маршрут, который НИЧЕГО не рендерит: у него нет разметки вовсе, а frontmatter
 * возвращает пустой ответ 404.
 *
 * Такой маршрут появился на задаче Э4-02 — `src/pages/[...missing].astro`. Он
 * существует затем, чтобы запрос к несуществующему адресу дошёл до middleware и
 * получил проверку по таблице редиректов; тело ответа при этом подставляет сам
 * Astro из единственной страницы 404. Требовать от него `BaseLayout` было бы
 * требованием ВТОРОЙ страницы 404 с другим телом.
 *
 * Исключение узкое и проверяемое: оно определяется не именем файла, а формой —
 * пустая разметка И пустой ответ 404. Маршрут, у которого появится хоть строка
 * разметки, немедленно вернётся в общую выборку и потребует навигацию.
 */
function neverRenders(file: string): boolean {
  const source = readFileSync(file, 'utf8');
  return (
    markupOf(source).trim() === '' &&
    /return new Response\(null, \{ status: 404 \}\);/u.test(source)
  );
}
...
describe('инварианты всех шаблонов страниц', () => {
  ...
  it('маршрутов без разметки ровно один — перехватывающий (задача Э4-02)', () => {
    // Список исключений не должен расти незаметно: каждый новый маршрут без
    // разметки — это страница без навигации, пока не доказано обратное.
    const silent = PAGE_TEMPLATES.filter(neverRenders).map((file) => file.replace(/^.*[\\/]/u, ''));

    expect(silent).toEqual(['[...missing].astro']);
  });

  it.each(RENDERING_TEMPLATES)('%s рендерит BaseLayout, то есть получает навигацию', (file) => {
