 ...
 * а ОТСУТСТВИЕ второго перехода: цель редиректа обязана отвечать сама, каким бы
 * ни был её статус. Ожидания на конкретный статус ставятся только там, где он от
 * наполнения базы не зависит.
 *
...
const CASES = [
  ...
  // Формы `<…>.html` СО СЛЕШЕМ. Класс путей, на котором 2026-08-28 контролёр
  // `url-guard` замерил регресс: запрос уходил в приложение сырым, и раньше
  // нашего middleware отвечал встроенный обработчик слеша Astro — 301 с телом, в
  // котором `<meta http-equiv="refresh">` указывал НАЗАД на источник (цикл для
  // клиента, предпочитающего meta заголовку), плюс чужой `<meta name="robots">`
  // и относительный canonical на ответе 3xx. Юнит-тест этого класса не ловил:
  // он проверяет решение, а сервер решение отбрасывал. Отсюда живые случаи.
  {
    path: '/o-proekte.html/',
    expect: 404,
    note: 'форма со слешем после расширения — 404 сразу, а не 301 с телом Astro',
  },
  {
    path: '/staraya.html/?utm_source=mail',
    expect: 404,
    note: 'адрес прежнего сайта со слешем: таблица переносов спрашивается только о форме без слеша',
  },
  { path: '/index.html/', expect: 404, note: 'второго адреса главной нет и в форме со слешем' },
  { path: '/404.html/', expect: 404, note: 'страница 404 недоступна как файл и со слешем' },
  {
    path: '/o-proekte.html//',
    expect: 404,
    note: 'пустой сегмент в хвосте того же класса путей',
  },
  { path: '/a.HTML/', expect: 404, note: 'регистр расширения решения не меняет' },
  {
    path: '/media/foo.html/',
    expect: 404,
    note: 'пространство /media тем же правилом: ни 301, ни тела 3xx',
  },
  {
    path: '/staraya.php/',
    expect: 404,
    note: 'симметрия: у «.html» со слешем нет привилегии перед соседним расширением',
  },
  {
    path: '/404',
    expect: 404,
  ...
  {
    path: '/robots.txt',
    // ОЖИДАНИЕ ОБНОВЛЕНО 2026-08-28: этап 4 дал маршрут, и файл отдаётся с 200.
    // Прежнее «404, маршрут появится на этапе 4» делало смоук красным на верном
    // коде — то есть глушило инструмент, которым ловится следующий регресс.
    expect: 200,
    note: 'URL файла отдаётся как есть и не нормализуется',
  },
  { path: '/robots.txt/', expect: 404, note: 'URL файла с завершающим слешем — не адрес' },
  // `/sitemap.xml` со статусом 200 здесь НЕ проверяется намеренно: индекс карты
  // сайта читает базу, и смоук правил URL стал бы падать от недоступной базы,
  // то есть по причине, к правилам URL отношения не имеющей. Отдачу sitemap
  // проверяет `smoke:sitemap` и приёмка. Форма со слешем базы не касается —
  // решение принимает политика пути, поэтому она здесь и остаётся.
  { path: '/sitemap.xml/', expect: 404, note: 'URL файла карты сайта слешем не нормализуется' },
  {
