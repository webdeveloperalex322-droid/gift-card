...
describe('вне режима обслуживания порядок прежний', () => {
  ...
  it('адрес на .html не отдаётся статикой, а уходит в приложение — за таблицей переносов', async () => {
    // Две половины одного требования (правка 2026-08-28):
    //   - файла заранее отрендеренной страницы по такому адресу НЕ отдаём, иначе
    //     у материала появился бы второй URL с 200;
    //   - и не отвечаем 404 сами: таблицу переносов читает только middleware
    //     Astro, а со структуры прежнего сайта переносят прежде всего `.html`.
    reachedAstro = [];
    const { captured, res } = fakeResponse();
    await frontDoor({})(fakeRequest('/o-proekte.html'), res);

    expect(reachedAstro).toEqual(['/o-proekte.html']);
    // Сам входной сервер ничего не ответил: ответ соберёт приложение.
  ...
  it('файл заранее отрендеренной страницы в сборке при этом остаётся нетронутым', async () => {
    ...
    // (`404.html` создан в beforeAll), и статический слой его не отдал.
    reachedAstro = [];
    const { captured, res } = fakeResponse();
    await frontDoor({})(fakeRequest('/404.html'), res);

    expect(reachedAstro).toEqual(['/404.html']);
