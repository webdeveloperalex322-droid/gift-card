 ...
 * группирующие узлы таксономии (`/otkrytki/prazdniki`, решение Ч-04-5) —
 * это ДАННЫЕ, а не маршруты, поэтому карточка со slug `prazdniki` даёт тот же
 * путь, что узел, и реестр такую коллизию не видит. Её закрывает проверка
 * уникальности итогового пути по всем коллекциям в хуках Payload (Э1-05 —
 * сборка пути подборки, Э1-09 — неизменяемость и уникальность slug).
 */
...
/**
 * Путь админки из окружения.
 *
 * Значения по умолчанию нет намеренно: дефолт означал бы, что при нестандартном
 * `PAYLOAD_ADMIN_PATH` реальный путь админки в резерв не попал, а в резерве
 * оказался путь, которого не существует. Тогда подборка с подходящим slug тихо
 * заняла бы адрес админки. Поэтому без параметра реестр отказывает.
 *
 * Хвостовой и ведущий слеш нормализуются здесь, а не в `.env`: это забота кода.
 */
function resolveAdminRoute(env: SharedEnv): RegistryEntry {
  const raw = env[PAYLOAD_ADMIN_PATH_ENV_KEY]?.trim() ?? '';
  if (raw === '') {
    throw new Error(
      `${PAYLOAD_ADMIN_PATH_ENV_KEY} не задан, поэтому путь админки нельзя внести в реестр ` +
        'зарезервированных маршрутов. Значения по умолчанию у него нет намеренно: при ' +
        'нестандартном пути дефолт зарезервировал бы не тот путь, и запись CMS заняла бы ' +
        'адрес админки (CLAUDE.md, правила URL).',
    );
  }
  ...
  if (segments.length === 0 || !segments.every((segment) => isValidSlug(segment))) {
    throw new Error(
      `${PAYLOAD_ADMIN_PATH_ENV_KEY} задан некорректно: «${raw}». Ожидается путь из ` +
        'сегментов [a-z0-9-] с ведущим слешем, например один сегмент верхнего уровня. ' +
        'Тихая нормализация здесь недопустима: путь админки участвует в резерве маршрутов.',
    );
  }
 ...
 * расширения».
 *
 * Порядок сборки важен: явные записи кладутся первыми, поэтому вид контейнера
 * побеждает производный резерв корневого сегмента. Иначе `/otkrytki`, чей
 * корневой сегмент совпадает с самим контейнером, оказался бы «занят целиком», и
 * весь каталог перестал бы принимать записи.
 *
...
export function reservedRoutes(env: SharedEnv = currentEnv()): readonly ReservedRoute[] {
  const entries: readonly RegistryEntry[] = [...STATIC_REGISTRY, resolveAdminRoute(env)];
