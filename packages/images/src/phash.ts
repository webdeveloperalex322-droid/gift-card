 ...
 *   - СРАВНЕНИЕ с порогом берёт `PHASH_DISTANCE_THRESHOLD`, а без настройки —
 *     утверждённое значение 14 (решение Ч-08, 2026-08-21). Отказ без настройки,
 *     который здесь был раньше, снят решением человека: дедупликация по ТЗ
 *     предупреждает редактора, а не удаляет и не блокирует, поэтому молчать
 *     из-за пустого `.env` она не должна. Мусорное значение при этом остаётся
 *     ошибкой: подменять его дефолтом — значит искать дубли не по тому порогу,
 *     который настроили.
 *
...
export interface PerceptualHashThresholdOptions {
  ...
  /**
   * Срез окружения для чтения `PHASH_DISTANCE_THRESHOLD`. Без переменной
   * берётся утверждённое значение (`DEFAULT_PHASH_DISTANCE_THRESHOLD` = 14).
   */
